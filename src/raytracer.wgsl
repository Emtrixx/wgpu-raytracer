...
//    let forward: vec3<f32> = normalize(cameraTarget - eye);
    ...
    let viewPointWorld: vec3<f32> = vec3<f32>((camera.rotation * vec4<f32>(viewPointLocal, 1.0)).xyz);
...
//    ray.origin = origin;
//    ray.direction = normalize(viewPointWorld.xyz - origin);

//    textureStore(color_buffer, globals.globalInvocationId.xy, vec4<f32>(ray.direction, 1.0));

    // Check for intersection with sphere
    let sphere_to_pixel: vec3<f32> = ray.origin - sphere_center;
    let a: f32 = dot(ray.direction, ray.direction);
    let b: f32 = 2.0 * dot(sphere_to_pixel, ray.direction);
